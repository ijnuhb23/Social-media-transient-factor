...
# section to obtain inverse volaility value for each sector


## import relevant files
dataretquart = pd.read_csv('C://Users//ijnuh//Desktop//JSV//Bayes//Term3//Project_AlternativeData//Python//Quarterly_All_StockweightedSectors_Returns//Quarterly_All_StockweightedSectors_Returns.csv')
...
# loop to obtain volatility of each sector
for i in sector_list:
    sector_name = str(i)

    dataretquart[str(sector_name)+' volatility'] = dataretquart[str(sector_name)+' returns'].var()

    dataretquart[str(sector_name)+' volatility'] = dataretquart[str(sector_name)+' volatility']**(1/2)

dataretquart

# save the volatiltiy of each sector
dataretquart.to_csv('C://Users//ijnuh//Desktop//JSV//Bayes//Term3//Project_AlternativeData//Python//volsectors//volsectors.csv')


## section to normalise the sector scores
...
# loop to store summary data from each sector into the main dataframe
for i in sector_list:
    sector_name = str(i)
    print(sector_name)
    ...
    # add the mean scores columns of each sector
...
## section to include the inverse volatility parameter


# loop to modify sectors scores with volatility adjustment
for i in sector_list:
    sector_name = str(i)

    for j in datanorm.index:

        datanorm.loc[j,'mean of '+ str(sector_name)] = datanorm.loc[j,'mean of '+ str(sector_name)]/dataretquart.loc[1,str(sector_name)+' volatility']

datanorm.to_csv('C://Users//ijnuh//Desktop//JSV//Bayes//Term3//Project_AlternativeData//Python//All_sectors_norm+inverse_vol//All_sectors_norm+inverse_vol.csv')
...
# save the results
datanormquart.to_csv('C://Users//ijnuh//Desktop//JSV//Bayes//Term3//Project_AlternativeData//Python//Sectors_to_long_short_inv_vol//All_Sectors_to_long_short_inv_vol.csv')
